...
class Product extends BaseModel
{
    ...
    {
        return $this->belongsTo('Media', 'amazon_screenshot', 'id');
    }
    ...
    public static function createOne($data)
    {
        ...
        try{
            if (count($data['img_id']) < 1) {
                ...
                $res = $ProductImageModel->createImg($data['img_id'], $prize->id);
                if ($res['error']) {
    ...
    public static function updateOne($data)
    {
        // 处理更新操作
        $ProductImageModel = new ProductImageModel();
        return $ProductImageModel->updateImg($data['img_id'], $data['id']);
    }
    ...
    public function deleteOne($id)
    {
        ...
        try{
            self::destroy($id);
            if (!ProductImageModel::destroy(['product_id' => $id])){
